...
| `LAKEBASE_CRED_403` | `Unauthorized network access to workspace <id>` — **403 on `/api/2.0/postgres/credentials` while OAuth from the same IP succeeded** | The credential-mint endpoint enforces a **Lakebase-specific network policy separate from the workspace IP ACL**. Allowlisting the workspace IP ACL is **necessary but not sufficient**. Capture the `request_id` and escalate to Databricks. See [Other ingress controls](#other-ingress-controls-beyond-ip-acl). |
