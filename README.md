...
- Separate workspaces for **analytics and OLTP** (one for Spark workloads, one
  hosting Lakebase) and analytics notebooks need to read application data
  without going through Unity Catalog federation.
- **Prototyping cross-workspace data access** before committing to a
  PrivateLink-backed architecture.
- Want a **service-principal-driven** read path (no user OAuth, no PATs).
